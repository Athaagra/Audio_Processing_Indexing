...
%      Version: $Id: ssubmmse.m 10481 2018-06-04 06:50:07Z dmb $
%
...
if isstruct(fsz)
    ...
    if nargin>=3 && ~isempty(pp)
        ...
        qp=struct;  % make an empty structure for estnoisem
    end
end
...
for ic=2:ncp                        % loop for each additional channel
    yf(:,:,ic)=rfft(enframe(s(:,ic),w,ni,rfm),nf,2);   % complex spectrum of channel ic
end
...
if isstruct(fsz)                    % check if we are following a previous call
    if ne>0                         % check noise estimation method
        [dp,ze]=estnoiseg(yp,ze);   % estimate the noise using MMSE in all frequency bins of nce channels
    else
...
if nr>0
    ...
    se=irfft(reshape(yf.*repmat(g,1,np),[nr nf2 nc*np]),nf,2).*repmat(w,[nr 1 nc*np]);     % inverse dft and apply output window
